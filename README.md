...
> The repository includes a `pnpm-lock.yaml`; use your preferred package manager in your environment.
...
Current `TypeOrmModule.forRoot(...)` config in `src/app.module.ts` expects:
...
- `synchronize: true` is enabled (helpful for local dev, usually avoided in production).
- JWT secret is currently hardcoded in `auth.module.ts` / `jwt.strategy.ts` for development simplicity.
